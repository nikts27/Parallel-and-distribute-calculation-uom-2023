...
def check_circuit(start, stop, size):
    
    # z: the combination number. Each process calculates values for z from start to stop
...
if __name__ == '__main__':
    ...
    # calculate the number of blocks that we divide the program
    block = iterations//num_processes
    ...
    for i in range(num_processes):
        ...
        if i == num_processes - 1:
            stop = iterations
        p = multiprocessing.Process(target=check_circuit, args=(start, stop, size,))
        processes.append(p)
